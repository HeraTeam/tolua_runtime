...
static lua_Integer lua_tointegerx(lua_State *L, int idx, int *isint)
{
    ...
    if (isint)
        *isint = (i != 0 || lua_type(L, idx) == LUA_TNUMBER);
    return i;
}
...
static lua_Number lua_tonumberx(lua_State *L, int idx, int *isnum)
{
    ...
    if (isnum)
        *isnum = (i != 0 || lua_type(L, idx) == LUA_TNUMBER);
    return i;
}
...
static void lpb_addlength(lua_State *L, pb_Buffer *b, size_t len)
{
    if (pb_addlength(b, len) == 0)
        luaL_error(L, "encode bytes fail");
}
...
static pb_Slice lpb_checkslice(lua_State *L, int idx)
{
    ...
    if (ret.p == NULL)
        typeerror(L, idx, "string/buffer/slice");
    return ret;
}
...
static uint64_t lpb_tointegerx(lua_State *L, int idx, int *isint)
{
...
#endif
    if ((os = s = lua_tostring(L, idx)) == NULL)
        return 0;
    while (*s == '#' || *s == '+' || *s == '-')
        neg = (*s == '-') ^ neg, ++s;
    ...
    if (*s != '\0')
        luaL_error(L, "integer format error: '%s'", os);
    *isint = 1;
    return neg ? ~v + 1 : v;
}
...
static int lpb_addtype(lua_State *L, pb_Buffer *b, int idx, int type, size_t *plen)
{
    ...
    case PB_Tdouble:
        ...
        if (ret)
            len = pb_addfixed64(b, pb_encode_double((double)v.lnum));
        if (v.lnum != 0.0)
            len = 0;
        break;
    case PB_Tfloat:
        v.lnum = lua_tonumberx(L, idx, &ret);
        if (ret)
            len = pb_addfixed32(b, pb_encode_float((float)v.lnum));
        if (v.lnum != 0.0)
            len = 0;
        break;
    case PB_Tfixed32:
    ...
    case PB_Tint32:
        v.u64 = lpb_tointegerx(L, idx, &ret);
        if (ret)
            len = pb_addvarint64(b, pb_expandsig((uint32_t)v.u64));
        if (v.u64 != 0)
            len = 0;
        break;
    case PB_Tuint32:
    ...
    case PB_Tsint32:
        v.u64 = lpb_tointegerx(L, idx, &ret);
        if (ret)
            len = pb_addvarint32(b, pb_encode_sint32(v.u32));
        if (v.u64 != 0)
            len = 0;
        break;
    case PB_Tfixed64:
    ...
    case PB_Tsint64:
        v.u64 = lpb_tointegerx(L, idx, &ret);
        if (ret)
            len = pb_addvarint64(b, pb_encode_sint64(v.u64));
        if (v.u64 != 0)
            len = 0;
        break;
    case PB_Tbytes:
    ...
    default:
        ...
        if (idx > 0)
            argcheck(L, 0, idx, lua_tostring(L, -1));
        lua_error(L);
    }
    if (plen)
        *plen = len;
...
#define pushinteger(n) lpb_pushinteger((L), (n), LS->int64_mode)
    ...
    case PB_Tsint64:
        ...
        {
        case PB_Tbool:
            lua_pushboolean(L, v.u64 != 0);
            break;
            /*case PB_Tenum:   pushinteger(v.u64); break; [> NOT REACHED <]*/
        case PB_Tint32:
            pushinteger((int32_t)v.u64);
            break;
        case PB_Tuint32:
            pushinteger((uint32_t)v.u64);
            break;
        case PB_Tsint32:
            pushinteger(pb_decode_sint32((uint32_t)v.u64));
            break;
        case PB_Tint64:
            pushinteger((int64_t)v.u64);
            break;
        case PB_Tuint64:
            pushinteger((uint64_t)v.u64);
            break;
        case PB_Tsint64:
            pushinteger(pb_decode_sint64(v.u64));
            break;
        }
        break;
    case PB_Tfloat:
    ...
    case PB_Tsfixed32:
        ...
        {
        case PB_Tfloat:
            lua_pushnumber(L, pb_decode_float(v.u32));
            break;
        case PB_Tfixed32:
            pushinteger(v.u32);
            break;
        case PB_Tsfixed32:
            pushinteger((int32_t)v.u32);
            break;
        }
        break;
    case PB_Tdouble:
    ...
    case PB_Tsfixed64:
        ...
        {
        case PB_Tdouble:
            lua_pushnumber(L, pb_decode_double(v.u64));
            break;
        case PB_Tfixed64:
            pushinteger(v.u64);
            break;
...
static int Lio_read(lua_State *L)
{
    ...
    if (fp != stdin)
        ...
        (void)setmode(fileno(stdin), O_TEXT);
    if (ret != LUA_OK)
    {
        lua_pushnil(L);
        lua_insert(L, -2);
...
static int lpb_packfmt(lua_State *L, int idx, pb_Buffer *b, const char **pfmt, int level)
{
    ...
    for (; *fmt != '\0'; ++fmt)
    {
        ...
        {
        case 'v':
            pb_addvarint64(b, (uint64_t)lpb_checkinteger(L, idx++));
            break;
        case 'd':
            pb_addfixed32(b, (uint32_t)lpb_checkinteger(L, idx++));
            break;
        case 'q':
            pb_addfixed64(b, (uint64_t)lpb_checkinteger(L, idx++));
            break;
        case 'c':
            pb_addslice(b, lpb_checkslice(L, idx++));
            break;
        case 's':
            pb_addbytes(b, lpb_checkslice(L, idx++));
            break;
        case '#':
            lpb_addlength(L, b, (size_t)lpb_checkinteger(L, idx++));
            break;
        case '(':
            len = pb_bufflen(b);
            ++fmt;
    ...
    if (level != 0)
        luaL_argerror(L, 2, "unmatch '(' in format");
    *pfmt = fmt;
    return idx;
}
...
static void lpb_enterview(lua_State *L, lpb_Slice *s, pb_Slice view)
{
    if (s->used >= s->size)
    {
        ...
        if (oldp == NULL)
            memcpy(newp, s->buff, s->used * sizeof(pb_Slice));
        s->buff = newp;
        s->size = newsize;
    }
...
static int lpb_unpackloc(lua_State *L, int *pidx, int top, int fmt, pb_Slice *s, int *prets)
{
    ...
    case '+':
        ...
        if (li == 0)
            li = 1;
        if (li > (lua_Integer)len)
            li = (lua_Integer)len + 1;
        s->p = s->start + li - 1;
        break;
...
static int Lslice_unpack(lua_State *L)
{
    ...
    if (s == NULL)
        view = lpb_checkslice(L, 1), s = &view;
...
static int lpb_pushfield(lua_State *L, const pb_Type *t, const pb_Field *f)
{
    ...
    lua_pushstring(L, f->type ? (const char *)f->type->name : pb_typename(f->type_id, "<unknown>"));
    lua_pushstring(L, (const char *)f->default_value);
    lua_pushstring(L, f->repeated ? (f->packed ? "packed" : "repeated") : "optional");
    if (f->oneof_idx > 0)
    {
...
static int lpb_pushdefault(lua_State *L, lpb_State *LS, const pb_Field *f, int is_proto3)
{
    ...
    case PB_Tfloat:
        if (f->default_value)
        {
            ...
            if ((const char *)f->default_value == end)
                return 0;
            ret = 1, lua_pushnumber(L, ln);
        }
        else if (is_proto3)
            ret = 1, lua_pushnumber(L, 0.0);
        break;
...
static int Lpb_defaults(lua_State *L)
{
    ...
    if (t == NULL)
        luaL_argerror(L, 1, "type not found");
...
static int Lpb_clear(lua_State *L)
{
    ...
    if (lua_isnoneornil(L, 2))
        pb_deltype(&LS->local, t);
    else
        pb_delfield(&LS->local, t, (pb_Field *)lpb_field(L, 2, t));
    LS->state = S;
    lpb_cleardefaults(L, LS, t);
    return 0;
}
...
static void lpbD_rawfield(lpb_Env *e, const pb_Field *f)
{
    ...
    case PB_Tenum:
        ...
        if (ev)
            lua_pushstring(L, (const char *)ev->name);
        else
            lpb_pushinteger(L, (lua_Integer)u64, default_lstate(L)->int64_mode);
        if (e->LS->use_hooks)
            lpb_usehooks(L, e->LS, f->type);
        break;
...
static void lpbD_repeated(lpb_Env *e, const pb_Field *f, uint32_t tag)
{
    lua_State *L = e->L;
    lpb_fetchtable(e, f);
    if (pb_gettype(tag) != PB_TBYTES || (!f->packed && pb_wtypebytype(f->type_id) == PB_TBYTES))
    {
        lpbD_field(e, f, tag);
...
{
    return lpb_decode(L, lua_isnoneornil(L, 2) ? pb_lslice(NULL, 0) : lpb_checkslice(L, 2), 3);
}
